...
- **A day's `preset` is what you asked for; `presetFor(rows)` is what you got.** They differ away from a barbell, the same way `config.intensity` of `auto` differs from the step it resolved to. Keeping the request is what lets a squat day survive a trip to the park and come back intact; reading the block back off the rows would quietly turn it into whatever the park allowed.
